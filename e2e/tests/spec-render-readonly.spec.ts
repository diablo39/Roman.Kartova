...
test("spec-render: API Definition tab renders the spec read-only (no live client)", async ({ page }) => {
  ...
  // suppressed by specRender.css (display:none) even though Scalar mounts them (#7741), so
  // they are present-but-hidden — assert on VISIBILITY. `:visible` count is 0 while hidden
  // and >0 if the CSS lock regresses; toHaveCount(0) on the bare selector would false-red
  // (elements exist) and wouldn't change when the lock is removed. The role-based button
  // check below already uses accessibility-tree visibility, so it needs no `:visible`.
