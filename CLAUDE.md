...
- **Definition of Done:** An implementation is "complete" / "finished" / "ready to merge" only when ALL of the following are green and can be cited by command + output:
  ...
  8. `/deep-review` skill run against the branch diff with spec / plan / ADRs / tests as context — produces a fixed-schema report (Blocking / Should-fix / Nits / Missing tests / What looks good). Blocking and Should-fix items addressed before merge; nits triaged.
