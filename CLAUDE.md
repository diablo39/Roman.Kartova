...
- **Definition of Done:** "complete" / "finished" / "ready to merge" only when the **ten always-blocking gates** are green and citable by command + output. Gate 6 (mutation) is **conditional** — blocking when the diff touches Domain/Application logic, else should-do. Order is fail-fast: cheap checks first, code-mutating gates before the final re-verify, and the running-system + CI gates (10–11) last.
  ...
  10. **Visual / API verification — observe the running system.** Drive the change end-to-end on the real stack, not just the suite: UI slices → cold-start the app, authenticate, navigate **in-SPA** (ADR-0084), screenshot the changed surface; API slices → exercise the **live** endpoint (real auth + DB) and capture the request/response. Evidence committed under `verification/<date>-<topic>/`. **Distinct from gate 3** (automated real-seam tests) — this observes the running system, which tests alone don't prove. Runs on the final, re-verified code. N/A (with reason) only when the diff has no runtime surface (docs / pure refactor).
