...
- **Definition of Done:** An implementation is "complete" / "finished" / "ready to merge" only when ALL of the following are green and can be cited by command + output:
  ...
  7. `/deep-review` skill run against the branch diff with spec / plan / ADRs / tests as context — produces a fixed-schema report (Blocking / Should-fix / Nits / Missing tests / What looks good). Blocking and Should-fix items addressed before merge; nits triaged.
  8. Mutation feedback loop run on changed files: `mutation-sentinel` (find surviving mutants) → `test-generator` (strengthen tests until mutants are killed). Mutation score must meet the repo target (≥80% per `stryker-config.json`). Document the score and any surviving mutants accepted as low-value.
  9. GitHub Copilot review requested on the PR (`gh pr edit <pr> --add-reviewer copilot-pull-request-reviewer`, or via the web UI). Copilot's findings reviewed and either addressed or explicitly dismissed with a reason in a PR comment.
