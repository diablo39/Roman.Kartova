...
- **Definition of Done:** "complete" / "finished" / "ready to merge" only when the **eight blocking gates** are green and citable by command + output. Gate 6 (mutation) is **should-do, not blocking**. Order is fail-fast: cheap checks first, code-mutating gates before the final re-verify.
  ...
  6. Mutation loop on changed files: `/misc:mutation-sentinel` → `/misc:test-generator`. **Should-do, not blocking** — too heavy to run every slice. Mandatory when the diff touches Domain/Application logic. Otherwise run when practical, or skip with a noted reason. Target ≥80% (`stryker-config.json`); document survivors.
