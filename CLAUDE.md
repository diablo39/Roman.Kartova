...
- **API entity model:** API is a first-class catalog entity; provider (`Api.implementedByApplicationId`) + instance (`Service.applicationId`) are nullable **FK fields** (structural, ADR-0110 precedent) — **not** graph edges; Service exposure of its app's APIs **derives** (full-auto); consumers are `consumes-api-from` **edges** targeting the API node; `ServiceEndpoint` = labeled address (protocol dropped → `Api.Style`) (ADR-0111, amends ADR-0068)
