...
import "./specRender.css";

type Props = { content: string; mediaType: string; rawFallback: ReactNode };
type State = { failed: boolean };

/**
 * Encapsulates the Scalar spec renderer (OpenAPI/Swagger and AsyncAPI — Scalar
 * auto-detects and renders both) behind an error boundary. Any parse or render
 * failure degrades to the raw source (ADR-0084: never blank-page) AND is logged
 * (so the degrade is never silent). Consumers reset the boundary after a failure
 * by keying it on the spec content (`key={content}`), so a corrected or replaced
 * spec gets a fresh instance instead of a stuck fallback.
 * Default export so ApiSpecSection can React.lazy() it and code-split the bundle.
 */
export default class SpecRender extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    ...
    console.error("[SpecRender] spec render failed; showing raw source", {
