...
(QUA-034)? Contract tests (see `knowledge/quality/test-strategy/what-to-test-at-which-level.md`)
are the enforcement mechanism — QUA-017 requires a contract change to update its test in the
same diff (`knowledge/quality/test-adequacy/contract-tests.md`); a silently changed contract is
an S1 finding even when all local tests pass. Schema changes coexist with the running version
per QUA-090 (`knowledge/quality/data-migration-safety/expand-contract.md`). The breaking-change
