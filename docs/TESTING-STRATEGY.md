...
- **The gates** (build / tests / container / reviews / DoD) live in [CLAUDE.md](../CLAUDE.md) → "Definition of Done." This doc never restates them — it explains how to *satisfy* the test-facing ones (gates 3 and 4).
...
- **Mutation testing:** **not a DoD gate** (removed 2026-08-02 — too heavy per slice for the defects it caught). Still available ad hoc via `/misc:mutation-sentinel` → `/misc:test-generator`; the Stryker configs (`≥80%` `thresholds.high`, generated by `scripts/generate-stryker-configs.py`) stay in place and CI keeps validating them.
