...
2. **One component.** When filters ARE built, they render through a shared `<FilterBar>` (react-aria-components + Tailwind v4 per ADR-0094), positioned above the `<DataTable>`. Standard control vocabulary only: **text search, single-select, multi-select, boolean toggle, date-range**. No bespoke per-screen filter chrome.
   - **(as-built 2026-06-23 — `multi-select` built; first consumers: Applications list lifecycle + team filters.)** The multi-select control (`components/base/multi-select/multi-select.tsx`) is a react-aria `DialogTrigger` → `Popover` → `ListBox selectionMode="multiple"` (checkmark per row). Because react-aria selection isn't a native form control, it **mirrors its selection into hidden `<input name=key>` elements** (rendered outside the portaled popover so they stay inside the `<form>`), keeping `<FilterBar>`'s uniform `FormData` commit path — multi-select keys are read via `data.getAll(key)`. It rides a new **`useListUrlState.multiFilters`** axis serialized as **repeated URL params** (`?lifecycle=active&lifecycle=deprecated`; empty ⇒ key absent, so the unfiltered cursor stays byte-identical). `useListFilters` derives multi-select keys to `queryFilters[key]: string[] | undefined`. The ADR-0095 cursor `f`-map value for a multi-select is the **sorted comma-joined** selection, present only when non-empty (order-independent identity). Controls are keyed by their committed values so Clear all / back-forward re-seed via `defaultSelectedKeys`. `date-range` remains the only reserved control type. (`<FilterBar>`/`useListFilters`/`useListUrlState` generics widened to a 4th `TMultiFilter`; the `urlState` `Pick` gains `multiFilters`.)
