...
security:
  ...
  browser_auth: SPA is a public OIDC client; KeyCloak access+refresh tokens held in browser `sessionStorage`, JWT Bearer to API. BFF (server-held tokens, HttpOnly cookie) is deferred as accepted risk — closes only token exfiltration, not XSS-rides-session; revisit on enterprise/compliance review. Interim hardening = short access TTL + refresh rotation w/ reuse-revocation + CSP (E-01.F-04.S-06) (ADR-0116)
agent:
...
domain_model:
  ...
  infrastructure_entity: Infrastructure is one discriminated `EntityKind` keyed by `InfrastructureType` (VM first), NOT a dedicated per-device kind; kind-agnostic aggregate = typed shared columns + opaque `jsonb attributes` (variant typing app-layer only, `VmAttributes`); generic vs type-specific `/vms` endpoint tiers; `@>` GIN-served filters; slice-1 sort = typed columns only (`displayName`/`type`/`createdAt`), JSONB-column sort + relationships (`PartOf`/`DeployedOn`) + `provider` deferred; perm `catalog.infrastructure.register` (ADR-0115)
...
| [0040](ADR-0040-two-view-dependency-graph-navigation.md) | Scoped Embedded Graph Views + One Standalone Explorer | Frontend Architecture | Accepted (amended 2026-08-07) | 0039 | Two *kinds* of graph surface, not a fixed count: scoped embedded views whose scope is each view's own property (mini-graph = 1 hop; System diagram = members + their edges), plus one full-screen `/graph` explorer with filters and `?focus=`. All share `EntityGraphNode`'s node grammar; a new embedded view must declare its scope and add no renderer of its own. Amendment adds the membership channel — boundary band + per-node `outsideBoundary` — and records that containment cannot be inferred from geometry. |
...
| [0111](ADR-0111-api-first-class-entity-provider-instance-fields.md) | API Is a First-Class Entity — Provider/Instance as FK Fields, Consumers as Edges, Exposure Derived | Domain Model | Accepted (Revised 2026-07-04; **Amended 2026-07-07, 2026-07-21, 2026-07-30**) | 0068 (amends), 0110, 0064, 0103, 0040, 0067, 0112, 0108 (extends) | API is a first-class team-owned entity, **one unified aggregate** keyed by `Style` (`Rest`/`Grpc`/`GraphQL`/`AsyncApi`) — async carries protocol/channel detail in the stored spec document (ADR-0112), not columns. Provider/instance/consumer links are all relationship edges (2026-07-04 revision); service↔service `depends-on` derives. **Amends ADR-0068**: `provides-api-for`/`consumes-api-from` target the API entity. `ServiceEndpoint` drops `Protocol`, gains optional `Description` (relaxed URL). **Amended 2026-07-21**: `PartOf` reintroduced for System grouping (`{Application,Service}→System`). **Amended 2026-07-30**: `PartOf` is **at-most-one per component**, enforced by the partial unique index `ux_relationships_one_system` (not write-time checks alone); `PUT /catalog/{applications|services}/{id}/system` is the canonical write path (`POST /relationships` is first-assign-only, 409 otherwise); a *move* mutates two edges so ADR-0108 authority applies **per mutated edge**; in the S-02 browse tree a System nests under its own steward team. |
