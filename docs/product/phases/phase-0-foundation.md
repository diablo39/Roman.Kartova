...
| E-01.F-01.S-04 | As a developer running the dev stack, I need an Org A organization row seeded into PostgreSQL so that `GET /api/v1/organizations/me` resolves for the realm-seeded `member@orga.kartova.local` user instead of returning 404 | Migrator (or a dedicated dev seed script) inserts an Organization row whose `tenant_id` matches the value in the KeyCloak realm seed; idempotent; opt-in via env var so it doesn't run in prod | [0085](../../architecture/decisions/ADR-0085-dedicated-migrator-container.md) |
...
| E-01.F-03.S-03 | As a developer, I need an append-only audit log table with tamper-evident design so that all write operations are accountable and tamper-evident (security forensics + GDPR accountability) | Audit table is insert-only (no UPDATE/DELETE); stores actor, action, timestamp, previous state; indexed for query performance; covers all write operations including entity CRUD, relationship changes (create/modify/delete/promote/demote), policy changes, and configuration changes | [0018](../../architecture/decisions/ADR-0018-append-only-tamper-evident-audit-log.md), [0106](../../architecture/decisions/ADR-0106-drop-regulatory-compliance-scope-gdpr-only.md) |
...
| Story ID | User Story | Acceptance Criteria | ADRs |
|----------|-----------|-------------------|------|
| E-01.F-05.S-01 | As an operator, I need a data retention engine that purges data older than the configured retention period so that storage is managed and GDPR is respected | Flat 180-day retention applied to all tenants; purge jobs run on schedule | [0017](../../architecture/decisions/ADR-0017-default-180-day-retention-5-year-mifid.md), [0106](../../architecture/decisions/ADR-0106-drop-regulatory-compliance-scope-gdpr-only.md) |
| ~~E-01.F-05.S-02~~ | ~~Tenant-level compliance flag (MiFID II regulated)~~ — **DROPPED (ADR-0106):** no regulatory tier, so no per-tenant compliance flag | — | [0106](../../architecture/decisions/ADR-0106-drop-regulatory-compliance-scope-gdpr-only.md) |
...
| ~~E-01.F-05.S-07~~ | ~~Notifications retained as MiFID II communication records (5 years)~~ — **DROPPED (ADR-0106):** notification log, if built, is an operational record under the flat 180-day window only | — | [0106](../../architecture/decisions/ADR-0106-drop-regulatory-compliance-scope-gdpr-only.md) |
