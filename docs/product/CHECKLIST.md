...
- [x] E-11.F-02.S-04 — API "Definition" tab (tabbed entity-detail layout) (tabbed-entity-detail, 2026-07-11): shared `DetailTabs` primitive (react-aria `Tabs`, ADR-0094) applied across all three detail pages — API: Overview · Dependencies · Definition (spec render, moved off Overview); Service / Application: Overview · Dependencies. Active tab tracked in `?tab=` (default `overview`, `replace` writes, invalid deep-link normalizes to default); only the active panel mounts, so the Definition Scalar chunk lazy-loads on open. ADR-0114. FU-1 (Playwright E2E for the tab-switch happy path) deferred. **FU-2 ([issue #71](https://github.com/diablo39/Roman.Kartova/issues/71)):** on the Dependencies tab the direct `provides-api-for`/`consumes-api-from` edges are shown twice — API-surface section (Provides/Consumes) **and** Relationships (Outgoing); pre-existing overlap (E-02.F-03 API surface + E-04 relationships) that the tab grouping made obvious. Proposed fix: exclude those two edge types from the Relationships list (API surface covers them richer). Touches ADR-0111 → design note before implementing; **handled separately**. Verification: `docs/superpowers/verification/2026-07-11-tabbed-entity-detail/dod.md`
