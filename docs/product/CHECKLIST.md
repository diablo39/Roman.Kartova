...
- [x] E-01.F-04.S-06 — Interim token-theft hardening (cheap alternative to S-05, ADR-0116). `sessionStorage`-over-`localStorage` already in place. Delivered in two sub-slices:
  ...
  - **S-06b — CSP on the web container** (commit `3ce22a9`, 2026-09-17): `Content-Security-Policy-Report-Only` via nginx envsubst template (`web/default.conf.template`), strict `script-src 'self'`; origins injected via `CSP_EXTRA_ORIGINS`. Ships **Report-Only** (non-breaking); drift-sentinel arch tests. DoD gates 1/2/3/4/8 green, 5 N/A, 6/7 owner-waived; ledger `docs/superpowers/verification/2026-09-17-web-csp-report-only/dod.md`. **Owner follow-up pending:** browser gate-9 (confirm 0 CSP violations) → **enforce-flip** (Report-Only → `Content-Security-Policy`); set `CSP_EXTRA_ORIGINS` on any future web k8s manifest.
