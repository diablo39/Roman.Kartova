...
- [ ] E-01.F-04.S-05 — BFF cookie-session auth (security hardening) — **DEFERRED, risk-accepted (2026-09-16)**. Current model = SPA public client, KeyCloak access+refresh tokens in `sessionStorage` (`web/src/shared/auth/authConfig.ts`), JWT Bearer to API. BFF (server-held tokens, HttpOnly+SameSite cookie) is current IETF BCP (`draft-ietf-oauth-browser-based-apps`) but **poor ROI now**: it closes only *token exfiltration*, not the larger XSS-rides-live-session risk (an XSS can make authenticated requests either way); cost is a ~4–6-slice, high-blast-radius migration (dual auth scheme, confidential KC client, multi-replica ticket store + shared DataProtection, same-origin/ingress + CORS removal, CSRF/antiforgery across all mutations, full FE auth cutover, real-seam test migration). **Not dropped** — kept tracked; **revisit trigger = enterprise/compliance security review**. Cheaper interim hardening carries most of the value → S-06.
- [x] E-01.F-04.S-06 — Interim token-theft hardening (cheap alternative to S-05, ADR-0116). `sessionStorage`-over-`localStorage` already in place. Delivered in two sub-slices:
  - **S-06a — KeyCloak refresh-token rotation + reuse-revocation** (commit `737ada5`, 2026-09-16): realm `revokeRefreshToken=true` + `refreshTokenMaxReuse=0` (rotated one-time-use refresh tokens; replay → whole session revoked). Access TTL already 900s. Drift-sentinel arch test. DoD gates 1/2/3(real-seam)/8 green, 4/5 N/A, 6/7 owner-waived; ledger `docs/superpowers/verification/2026-09-16-keycloak-refresh-rotation/dod.md`. **Owner actions pending:** apply same to prod KeyCloak (Realm → Tokens → Revoke Refresh Token ON, Max Reuse 0); gate-9 (running-stack silent-renew) + gate-10 (CI).
  - **S-06b — CSP on the web container** (commit `3ce22a9`, 2026-09-17): `Content-Security-Policy-Report-Only` via nginx envsubst template (`web/default.conf.template`), strict `script-src 'self'`; origins injected via `CSP_EXTRA_ORIGINS`. drift-sentinel arch tests. DoD gates 1/2/3/4/8 green, 5 N/A, 6/7 owner-waived; ledger `docs/superpowers/verification/2026-09-17-web-csp-report-only/dod.md`. **FU-CSP-1 done (2026-09-17):** gate-9 probe (`e2e/csp-check.mjs`) found Scalar the only real blocker → `withDefaultFonts:false` + `telemetry:false` + `api.scalar.com` allowlisted in connect-src; **flipped to ENFORCING** (`Content-Security-Policy`), verified 0 real violations under enforcement across all SPA surfaces. Remaining owner note: set `CSP_EXTRA_ORIGINS` on any future web k8s manifest (helm has no web deployment today).
...
- [x] E-02.F-04.S-01 — Register infrastructure components — **slice 1 (read + create) merged 2026-09-08 (PR #86, ADR-0115)**: discriminated `EntityKind.Infrastructure` keyed by `InfrastructureType` (VM first), kind-agnostic aggregate + opaque indexed-JSONB attributes, generic vs type-specific `/vms` endpoint tiers, `catalog.infrastructure.register` perm, VM list/detail/register + All-Objects list. All ten DoD gates green — ledger `docs/superpowers/verification/2026-09-08-infrastructure-vm-slice1/dod.md`, spec `docs/superpowers/specs/2026-09-08-infrastructure-vm-slice1-design.md`. **Slice 2a (VM fields + mutation) merged 2026-09-09**: `provider` field (column + sort on both Infrastructure and VM lists, filter deferred), VM update (optimistic-concurrency, ETag/`If-Match`) + delete, `VmSortField`/`VmSortSpecs` JSONB-column sort (`powerState`, `os`, `vcpu`, `memoryGb`, `hostname`, `region`) added to the VM sort allowlist — filter set unchanged from slice 1. Spec `docs/superpowers/specs/2026-09-09-infrastructure-vm-slice2a-fields-mutation-design.md`, plan `docs/superpowers/plans/2026-09-09-infrastructure-vm-slice2a-fields-mutation.md`, ledger `docs/superpowers/verification/2026-09-09-infrastructure-vm-slice2a/dod.md`. **Slice 2b (VM linking) — PR #91, 2026-09-16, all 10 DoD gates green:** `DeployedOn` ({Application,Service}→VM, VM-only) + `PartOf` (Infrastructure→System, at-most-one via reused `SetComponentSystemAsync` + new `PUT /catalog/infrastructure/{id}/system`) relationship edges; `CatalogEntityLookup` resolves `EntityKind.Infrastructure`; kind-agnostic graph traversal renders infra nodes + both edges; FE `DeployOnVmDialog`/`DeployOnVmAction` + VM-detail System-membership + Hosted-components + infra graph node. No new permission. Ledger `docs/superpowers/verification/2026-09-15-catalog-vm-linking/dod.md`, spec `docs/superpowers/specs/2026-09-15-catalog-vm-linking-design.md`. Follow-ups: TD-005 (infra in `/catalog/hierarchy`) + TD-006 (SystemMembersSection infra members) **done (#93, 2026-09-16)**; TD-007 (`ListVms` `displayNameContains`) + TD-009 (System-side Infrastructure assign) + TD-008 (FE vitest flake mitigation) **done (branch `feat/catalog-vm-system-assign-search`, 2026-09-16)**; TD-010 (VM `limit` OpenAPI transformer gap) filed open. S-01 fully done; broker registration = S-02.
...
- [x] E-02.F-05.S-01 — Register environments with infra details — **sub-slice A1 (register + list + get) merged 2026-09-18, [ADR-0117](../architecture/decisions/ADR-0117-environment-and-deployment-entities.md)**: `EntityKind.Environment`, tenant-global not team-owned (`ITenantOwned` only, deliberate ADR-0103 deviation), `catalog.environments.register` perm (Member+OrgAdmin, 5-sync), POST/GET-by-id/cursor-list at `/api/v1/catalog/environments`, list screen + detail + register dialog + nav. Sort allowlist `{displayName (default asc), createdAt, type, region}`; filters `type` (multi-select), `region` (exact), `displayNameContains` — registry row: `docs/design/list-filter-registry.md`. Real-seam integration 9/9 green, unit+arch 75/75 green; ledger `docs/superpowers/verification/2026-09-18-environment-register-list/dod.md`. Resource-details capture UI deferred (API/storage support it; register dialog collects name/type/region only in A1). `Cluster` field removed pre-merge (2026-09-23). **Sub-slice A2 (edit + delete) — 2026-09-24**: `CatalogEnvironment.Edit` domain method — metadata edit only, `Type` immutable (per design doc; Delete has no lifecycle guard, Environment has no owning team); `PUT/DELETE /api/v1/catalog/environments/{id}`, If-Match/ETag optimistic concurrency (ADR-0096 pattern), mirrors the VM edit/delete slice. Edit gated by its own dedicated OrgAdmin+Member `catalog.environments.edit` permission (not a reuse of Register — an initial version reused Register, caught and fixed by a gate-2 spec-compliance review); Delete gated by OrgAdmin-only `catalog.environments.delete` (both 5-sync: `KartovaPermissions.cs`, `KartovaRolePermissions.cs`, `permissions.snapshot.json`, `permissions.ts`, `usePermissions.test.tsx`). Rename repeats Register's per-tenant display-name uniqueness check, now factored into a shared `CheckEnvironmentNameAvailableAsync`/`IsEnvironmentNameConflictRace` helper used by both Register and Edit (pre-check + `23505` backstop → 409 `environment-name-conflict`). `EditEnvironmentDialog`/`DeleteEnvironmentConfirm` + shared `EnvironmentFormFields` (extracted from `RegisterEnvironmentDialog`, with a `beforeRegion` slot so only Register renders the Type select) on the detail page, gated per-button on the two permissions. Real-seam integration tests (37 total, all green) + domain unit tests (26) + frontend component/page tests; browser-verified end-to-end (edit + delete round-trip, 0 console errors) against the live stack. DoD ledger: `docs/superpowers/verification/2026-09-24-environment-edit-delete/dod.md`. Deferred, tracked in the spec's slice-B row: DELETE's `409 environment-has-deployments` guard (unreachable today — no Deployment table exists yet).
...
- [x] E-04.F-01.S-05 — De-duplicate API provide/consume edges from the Relationships list ([issue #71](https://github.com/diablo39/Roman.Kartova/issues/71)) — on Application/Service detail pages the **direct** `provides-api-for`/`consumes-api-from` edges appear in BOTH the API-surface section (Provides/Consumes) and Relationships (Outgoing). Pre-existing overlap (E-02.F-03 API surface + E-04 relationships) made obvious by the E-11.F-02.S-04 tabbed layout grouping both under one Dependencies tab. Proposed fix: exclude those two edge types from the Relationships list (API surface covers them richer — style/version/spec + derived). Touches ADR-0111 → brainstorm/design note before implementing. (relationships-api-edge-dedup, 2026-07-13; resolves #71): server-side `excludeApiEdges` flag on `GET /catalog/relationships`, set by App/Service `RelationshipsSection` (`variant="full"`); the richer API-surface section is now the sole home of direct provides/consumes edges. API detail page (`variant="incoming-only"`) unaffected — still lists providers/consumers. Backend query filter applied pre-pagination (pager stays honest). ADR-0114 amended with the Overview→Dependencies→rest tab-order convention.
...
- [x] E-11.F-02.S-01 — Render OpenAPI specs as interactive docs (openapi-spec-render, 2026-07-10; PR #69): read-only **Scalar** (`@scalar/api-reference-react`) render replacing the raw `<pre>` on the API detail page. Content-detected OpenAPI/Swagger (`detectSpecKind`, top-level-key-only) → rendered by default with a **Rendered⇄Raw toggle**; every non-OpenAPI/malformed spec (gRPC/GraphQL/AsyncAPI/garbage) keeps the raw view. `OpenApiRender` encapsulates Scalar behind a logging error-boundary (never blank-page, ADR-0084), lazy-loaded so the ~2.8 MB chunk stays out of the main bundle (Tailwind-v4 `@layer` order per ADR-0094). **Read-only enforced**: `hideClientButton` + `hideTestRequestButton` + scoped CSS hiding Scalar's interactive API client — gate-10 browser verification caught that `hideTestRequestButton` alone leaves the live "Send Request" client reachable (Scalar #7741) and fixed it. Frontend-only (no backend/contract/permission change); gate-5 real-seam + gate-6 mutation **N/A**. All blocking DoD gates green (build/suite 811, web image, /simplify, reviews 2/7/8/9, gate-10 browser: render/toggle/AsyncAPI-fallback/XSS-sanitized/0-console-errors, CI #69). **AsyncAPI folded in (see F-03.S-01)** — same `SpecRender` component (renamed from `OpenApiRender`; folder `components/spec/`); GraphQL/gRPC rendering + try-it-out still deferred (F-02.S-02). FU-1: Playwright E2E regression for the read-only lock — **DONE 2026-07-20** (e2e-spec-render-tabs: `e2e/tests/spec-render-readonly.spec.ts` asserts the Definition tab renders read-only with the live Scalar client suppressed; DevSeed API+OpenAPI fixture; 2 passed on the real stack). Verification: `docs/superpowers/verification/2026-07-10-openapi-spec-render/dod.md`. **New FU (from that slice's gate-8): AsyncAPI read-only-lock E2E** — the CSS lock claims OpenAPI+AsyncAPI but only OpenAPI is E2E-tested; needs an AsyncAPI fixture + spec.
- [ ] E-11.F-02.S-02 — Render gRPC proto files as browsable docs
- [ ] E-11.F-02.S-03 — Versioned API docs aligned with deployments
- [x] E-11.F-02.S-04 — API "Definition" tab (tabbed entity-detail layout) (tabbed-entity-detail, 2026-07-11): shared `DetailTabs` primitive (react-aria `Tabs`, ADR-0094) applied across all three detail pages — API: Overview · Dependencies · Definition (spec render, moved off Overview); Service / Application: Overview · Dependencies. Active tab tracked in `?tab=` (default `overview`, `replace` writes, invalid deep-link normalizes to default); only the active panel mounts, so the Definition Scalar chunk lazy-loads on open. ADR-0114. FU-1 (Playwright E2E for the tab-switch happy path) — **DONE 2026-07-20** (e2e-spec-render-tabs: `e2e/tests/detail-tabs.spec.ts` asserts tab-switch + `?tab=` sync + only-active-panel-mounts + invalid-`?tab` normalization; 2 passed on the real stack). FU-2 (provides/consumes vs Relationships duplication on the Dependencies tab) tracked standalone as **E-04.F-01.S-05** / [issue #71](https://github.com/diablo39/Roman.Kartova/issues/71). Verification: `docs/superpowers/verification/2026-07-11-tabbed-entity-detail/dod.md`
