...
- [x] E-02.F-05.S-01 — Register environments with infra details — **sub-slice A1 (register + list + get) merged 2026-09-18, [ADR-0117](../architecture/decisions/ADR-0117-environment-and-deployment-entities.md)**: `EntityKind.Environment`, tenant-global not team-owned (`ITenantOwned` only, deliberate ADR-0103 deviation), `catalog.environments.register` perm (Member+OrgAdmin, 5-sync), POST/GET-by-id/cursor-list at `/api/v1/catalog/environments`, list screen + detail + register dialog + nav. Sort allowlist `{displayName (default asc), createdAt, type, region}`; filters `type` (multi-select), `region` (exact), `displayNameContains` — registry row: `docs/design/list-filter-registry.md`. Real-seam integration 9/9 green, unit+arch 75/75 green; ledger `docs/superpowers/verification/2026-09-18-environment-register-list/dod.md`. Resource-details capture UI deferred (API/storage support it; register dialog collects name/type/region only in A1). `Cluster` field removed pre-merge (2026-09-23). **Sub-slice A2 (edit + delete) — 2026-09-24**: `CatalogEnvironment.Edit` domain method (full-replacement, no immutable field — Environment has no owning team to protect); `PUT/DELETE /api/v1/catalog/environments/{id}`, If-Match/ETag optimistic concurrency (ADR-0096 pattern), mirrors the VM edit/delete slice. Edit reuses the `CatalogEnvironmentsRegister` claim; Delete gated by new OrgAdmin-only `catalog.environments.delete` permission (5-sync: `KartovaPermissions.cs`, `KartovaRolePermissions.cs`, `permissions.snapshot.json`, `permissions.ts`, `usePermissions.test.tsx`). Rename repeats Register's per-tenant display-name uniqueness check (pre-check + `23505` backstop → 409 `environment-name-conflict`). `EditEnvironmentDialog`/`DeleteEnvironmentConfirm` + shared `EnvironmentFormFields` (extracted from `RegisterEnvironmentDialog`) on the detail page, gated per-button on the two permissions. Real-seam integration tests (31 total, all green) + domain unit tests (22) + frontend component/page tests; browser-verified end-to-end (edit + delete round-trip, 0 console errors) against the live stack.
