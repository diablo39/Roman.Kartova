...
- [ ] E-01.F-04.S-06 — Interim token-theft hardening (cheap alternative to S-05): short access-token TTL + **refresh-token rotation with reuse-revocation** in KeyCloak (config, near-zero code — shrinks stolen-token window to minutes, self-destructs stolen refresh tokens) + strong **CSP** header (shrinks XSS surface directly). `sessionStorage`-over-`localStorage` already in place.
