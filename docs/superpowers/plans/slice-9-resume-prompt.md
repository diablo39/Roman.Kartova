...
- **D-phase Nits carried forward** (consider in Phase H mutation-sentinel pass, not Phase D scope):
  ...
  8. ~~Add `Content-Security-Policy` header on the SVG serving path~~ ✅ **ADDRESSED in `<commit-sha>`** — CSP `default-src 'none'; style-src 'unsafe-inline'; sandbox` + `X-Content-Type-Options: nosniff` set on both 304 and 200 paths of `GetLogoAsync`. (Triggered by automated security review.) If a future slice serves more binary endpoints, consider lifting these to a global middleware.
