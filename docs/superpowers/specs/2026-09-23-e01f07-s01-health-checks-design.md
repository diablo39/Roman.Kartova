...
| File | Responsibility |
|---|---|
| `src/Kartova.Api/HealthChecks/KeycloakHealthCheck.cs` | `IHealthCheck`. HTTP GET `{BaseUrl}/realms/{Realm}/.well-known/openid-configuration` via a named `HttpClient` (`AddHttpClient(KeycloakHealthCheck.ClientName, ...)`), reusing `KeycloakAdminOptions` (`Kartova.SharedKernel.Identity`) for `BaseUrl`/`Realm`. Catches all exceptions internally → `Unhealthy(description, ex)`, never throws. Tags `["ready","startup"]`. |
