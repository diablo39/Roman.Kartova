...
- **JSONB-sort null-key COALESCE** (deep-review gate-8 finding, deferred to slice-4): the 6 VM JSONB sort selectors are **not** COALESCE-guarded (unlike `Provider ?? ""`). Safe in 2a because `VmAttributes.Validate` guarantees all attribute keys present + non-null on every create/edit write. **Slice-4 auto-import**, which writes rows bypassing `VmAttributes.Validate` (§3 #4), MUST either validate the attribute set on ingest or add COALESCE to these selectors **and** re-match the 6 partial indexes byte-for-byte. Documented as an invariant in `VmSortSpecs`. Not fixed in 2a: re-touching the byte-matched selector/index/EXPLAIN machinery for an unreachable case is higher risk than the guard.
