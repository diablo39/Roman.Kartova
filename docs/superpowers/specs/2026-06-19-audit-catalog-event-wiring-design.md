...
| # | Decision | Rationale |
|---|----------|-----------|
| 1 | **`IAuditWriter` flows through the existing direct-dispatch call path** — add `IAuditWriter audit` to each `Handle(...)` signature AND to the corresponding minimal-API delegate in `CatalogEndpointDelegates.cs`, which resolves it from ASP.NET Core DI and passes it through. | *(Corrected during implementation — the original draft said "Wolverine resolves method parameters." Catalog handlers are NOT Wolverine-bus-dispatched; per ADR-0093 they are called directly from minimal-API delegates that already resolve `CatalogDbContext`/`ITenantContext`/`ICurrentUser` via endpoint DI. So `IAuditWriter` is one more DI-resolved delegate parameter, threaded to `Handle`.)* Avoids constructor churn and matches Catalog's existing delegate→handler style. Any direct-call test (no HTTP principal) must supply an `IAuditWriter` whose `ICurrentUser` has a valid actor. |
