...
- **Fix:** Re-run `/misc:mutation-sentinel` against the current Domain tree, confirm ≥80% (and the claimed 100%), and commit the regenerated `mutation-report-surviving.md` (or record the score in the slice evidence file `docs/superpowers/verification/2026-06-12-audit-log-foundation/db-verification.md`). One of the two stale survivors (the truncation-direction arithmetic mutant) maps to a still-relevant test gap — see Missing tests.
