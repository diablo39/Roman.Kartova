...
| Systems | `GET /catalog/systems` (backend only — **no UI this slice**, ADR-0111 amendment 2026-07-21 DRAFT) | `displayNameContains` (text) + `teamId` (multi-select) | **built (API-only, no `<FilterBar>` yet)** | E-03.F-03.S-01 | Sort allowlist `{displayName, createdAt}`, default `displayName asc` (standard). Filters mirror the `ListApis` shape minus `style` (Systems have no style dimension). Deferred: `createdAt` range (reserved control, ADR-0107 clause 1, no screen needs it yet) and `memberCount` (needs a `PartOf`-edge aggregate join — deferred alongside the browse-by-hierarchy story, E-03.F-03.S-02). No `<FilterBar>`/`useListFilters` wiring this slice — backend-first per the plan; revisit this row when the Systems list screen is built. |
