...
**Status:** done (branch `chore/tech-debt-td-012`) — `ConcurrencyTokenCapture.SetExpectedVersion(EntityEntry, uint)` added, sharing a private `FindConcurrencyTokenProperty` lookup with the read-side `TryCaptureCurrentVersionAsync` (TD-002); throws on a missing or non-`uint` token property instead of silently boxing a mismatch, since this call is load-bearing for the guard. All 5 call sites (`EditApplicationHandler`, `EditVmHandler`, `DeleteVmHandler`, `EditEnvironmentHandler`, `DeleteEnvironmentHandler`) migrated off their hard-coded `Xmin`/`Version` literals. Unit-tested (`ConcurrencyTokenCaptureTests`, in-memory EF model) + covered end-to-end by each handler's existing real-seam stale-version → 412 integration test. DoD ledger: `docs/superpowers/verification/2026-09-25-td-012-concurrency-write-side/dod.md`.
